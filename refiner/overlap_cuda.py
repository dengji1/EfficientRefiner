...
def cal_o_dp(o, dp, pos, size):
    ...
    for idx1 in range(ix, node_cnt, threads_per_grid_x):
        for idx2 in range(iy, node_cnt, threads_per_grid_y):
            if idx1 != idx2 and -(size[idx1] + size[idx2]) / 2 < pos[idx1] - pos[idx2] < (size[idx1] + size[idx2]) / 2:
...
def get_overlap(chip : RefineDB, pos, args):
    ...
    cal_o_dp[blocks_per_grid_2d, threads_per_block_2d](ox_gpu, dpx_gpu, pos_x_gpu, size_x_gpu)
    ...
    cal_o_dp[blocks_per_grid_2d, threads_per_block_2d](oy_gpu, dpy_gpu, pos_y_gpu, size_y_gpu)

    overlap_gpu = cuda.device_array(shape=(1), dtype=np.float32)
    overlap_gpu.copy_to_device(np.zeros((1), dtype=np.float32))
    cal_overlap[blocks_per_grid_2d, threads_per_block_2d](overlap_gpu, ox_gpu, oy_gpu)
    ...
    cal_dpos[blocks_per_grid_2d, threads_per_block_2d](dposx_gpu, oy_gpu, dpx_gpu)
    ...
    cal_dpos[blocks_per_grid_2d, threads_per_block_2d](dposy_gpu, ox_gpu, dpy_gpu)
